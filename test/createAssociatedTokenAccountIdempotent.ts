import { Address, assertAccountExists, fetchEncodedAccount, generateKeyPairSigner } from '@solana/kit';
...
test('it creates tokenkeg associated token account', async (t) => {
  ...
  const result = await createAndConfirmAssociatedTokenAccount(client.rpc, client.rpcSubscriptions, payer, owner.address, mint, true);


  const account = await fetchEncodedAccount(client.rpc, result.associatedTokenAddress);
  ...
  let result2: { associatedTokenAddress: Address, signature: string } | undefined = undefined;
  await t.notThrowsAsync(async () => {
    result2 = await createAndConfirmAssociatedTokenAccount(client.rpc, client.rpcSubscriptions, payer, owner.address, mint, true);
  });
  t.truthy(result2);
  
  let result3: { associatedTokenAddress: Address, signature: string } | undefined = undefined;
  await t.throwsAsync(async () => {
    result3 = await createAndConfirmAssociatedTokenAccount(client.rpc, client.rpcSubscriptions, payer, owner.address, mint, false);
  });
...
test('it creates tokenZ associated token account', async (t) => {
  ...
  const ata = await createAndConfirmAssociatedTokenAccount(client.rpc, client.rpcSubscriptions, payer, owner.address, mint, true);
